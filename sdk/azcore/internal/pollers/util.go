...
func PollHelper(ctx context.Context, endpoint string, pl pipeline.Pipeline, update func(resp *http.Response) (string, error)) error {
	req, err := pipeline.NewRequest(ctx, http.MethodGet, endpoint)
	if err != nil {
		return err
	}
	resp, err := pl.Do(req)
