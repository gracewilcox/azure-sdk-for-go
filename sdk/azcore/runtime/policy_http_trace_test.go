...
func TestHTTPTracePolicy(t *testing.T) {
	...
	pl := pipeline.New(srv, newHTTPTracePolicy([]string{"visibleqp"}))
	...
	// HTTP ok
	req, err = pipeline.NewRequest(context.WithValue(context.Background(), shared.CtxWithTracingTracer{}, tr), http.MethodGet, srv.URL()+"?foo=redactme&visibleqp=bar")
	require.NoError(t, err)
