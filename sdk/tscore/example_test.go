...
import (
	...
	"github.com/Azure/azure-sdk-for-go/sdk/tscore/log"
...
func (w Widget) MarshalJSON() ([]byte, error) {
	...
	if tscore.IsNullValue(w.Name) {
		msg["name"] = nil
	} else if w.Name != nil {
		msg["name"] = w.Name
	}
	if tscore.IsNullValue(w.Count) {
		msg["count"] = nil
	} else if w.Count != nil {
		msg["count"] = w.Count
	}
...
func ExampleResponseError() {
	pipeline := runtime.NewPipeline(runtime.PipelineOptions{}, nil)
